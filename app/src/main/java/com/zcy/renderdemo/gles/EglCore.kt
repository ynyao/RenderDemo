 ...
 */
@JvmOverloads constructor(sharedContext: EGLContext? = null, flags: Int = 0, usePbuffer: Boolean = false) {
    ...
    init {
        ...
        if (flags and FLAG_TRY_GLES3 != 0) {
            ...
            if(usePbuffer){
                config = getPbufferConfig(flags, 3)
            }else{
        ...
        if (mEGLContext === EGL14.EGL_NO_CONTEXT) {  // GLES 2 only, or GLES 3 attempt failed
            ...
            var config: EGLConfig? = null
            if(usePbuffer){
                config = getPbufferConfig(flags, 2) ?: throw RuntimeException("Unable to find a suitable EGLConfig")
            }else{
                config = getConfig(flags, 2) ?: throw RuntimeException("Unable to find a suitable EGLConfig")
            }
        ...
        EGL14.eglQueryContext(mEGLDisplay, mEGLContext, EGL14.EGL_CONTEXT_CLIENT_VERSION,
                values, 0)
        L.d("EGLContext created, client version " + values[0])
    }

    private fun getPbufferConfig(flags: Int, version: Int): EGLConfig?{
        ...
        if (!EGL14.eglChooseConfig(mEGLDisplay, attribList, 0, configs, 0, configs.size,
                        numConfigs, 0)) {
            L.e("unable to find RGB8888 / $version EGLConfig")
            return null
        }
        return configs[0]
    }
    ...
    protected fun finalize() {
        try {
            if (mEGLDisplay !== EGL14.EGL_NO_DISPLAY) {
                ...
                L.e( "WARNING: EglCore was not explicitly released -- state may be leaked")
                release()
            }
        } finally {
        }
    ...
    fun makeCurrent(eglSurface: EGLSurface) {
        if (mEGLDisplay === EGL14.EGL_NO_DISPLAY) {
            ...
            L.d("NOTE: makeCurrent w/o display")
        }
    ...
    fun makeCurrent(drawSurface: EGLSurface?, readSurface: EGLSurface?) {
        ...
        if (!EGL14.eglMakeCurrent(mEGLDisplay, drawSurface?: EGL14.EGL_NO_SURFACE, readSurface?: EGL14.EGL_NO_SURFACE, mEGLContext)) {
    ...
    companion object {
        ...
        fun logCurrent(msg: String) {
            ...
            L.d("Current EGL (" + msg + "): display=" + display + ", context=" + context +
                    ", surface=" + surface)
        }
    }
}
